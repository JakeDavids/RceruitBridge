...
import {
  ...
  Plus,
  AlertCircle,
  LayoutDashboard // Added LayoutDashboard import
} from "lucide-react";
...
import ShareWithTeam from "../components/dashboard/ShareWithTeam";
import OnboardingWalkthrough from "../components/onboarding/OnboardingWalkthrough"; // Assuming this is the path to your new component
...
export default function Dashboard() {
  ...
  useEffect(() => {
    // CRITICAL: Only show walkthrough if:
    // 1. User has NEVER seen it before (localStorage flag is missing or false)
    // 2. AND this is likely their first visit (no athlete profile yet)
    
    const hasSeenOnboarding = localStorage.getItem('rB_seenOnboarding');
    
    if (!hasSeenOnboarding || hasSeenOnboarding === 'false') {
      // Check if user is brand new (no athlete profile)
      const checkIfNewUser = async () => {
        try {
          ...
          if (user) {
            ...
            // Only show if no athlete profile exists (brand new user)
            if (athleteData.length === 0) {
              setTimeout(() => {
                setShowOnboarding(true);
              }, 1000);
            } else {
              // User has profile but flag wasn't set - set it now
              localStorage.setItem('rB_seenOnboarding', 'true');
            }
          }
        } catch (err) {
          console.error('Error checking new user status for onboarding:', err);
        }
      };
  ...
  const loadDashboardData = async () => {
    try {
      ...
      try {
        ...
        setError("You must be logged in to view the dashboard. Redirecting to login...");
        setLoading(false);
        setTimeout(() => {
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 p-4 md:p-6 pt-16 lg:pt-6">
        <motion.div
        ...
        >
          ...
          <motion.div variants={containerVariants} className="flex flex-col gap-4 bg-white/80 backdrop-blur-sm rounded-xl md:rounded-2xl p-4 md:p-6 shadow-lg border border-slate-200">
            <div className="flex items-start gap-3 md:gap-4">
              <div className="w-12 h-12 md:w-16 md:h-16 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl md:rounded-2xl flex items-center justify-center shadow-lg flex-shrink-0">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
              {currentUser && athlete && (
                <ShareWithTeam 
                  user={currentUser} 
                  referralCode={currentUser.referral_code || `${athlete.first_name}${athlete.graduation_year}`.toUpperCase().replace(/\s/g, '')}
                />
              )}
              {!athlete && (
              ...
              <Card className="border-l-4 border-l-amber-500 bg-amber-50">
                <CardContent className="pt-4 md:pt-6">
                  <div className="flex flex-col sm:flex-row items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-semibold text-amber-900 text-base md:text-lg">Complete Your Profile</h3>
          ...
          {/* Stats Grid - Better mobile stacking */}
          ...
          <motion.div variants={containerVariants} className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
